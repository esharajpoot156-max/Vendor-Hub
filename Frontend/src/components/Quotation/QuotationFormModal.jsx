    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-brand-950/50 px-4 backdrop-blur-sm">
      <div className="animate-fade-up w-full max-w-md rounded-xl bg-white p-6 shadow-2xl">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="font-display text-lg font-semibold text-brand-950">New Quotation Request</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="mb-1 block text-sm font-medium text-slate-700">Quotation Title</label>
            <input
              type="text"
              value={form.title}
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm outline-none transition-all duration-200 focus:border-brand-700 focus:ring-2 focus:ring-brand-700/15"
            />
          </div>
          ...
          <div>
            ...
            <div className="max-h-40 space-y-1 overflow-y-auto rounded-md border border-slate-300 p-2">
              {vendors.length === 0 ? (
                <p className="px-2 py-1 text-sm text-slate-500">No vendors found. Add a vendor first.</p>
              ) : (
                vendors.map((v) => (
                  <label
                    key={v._id}
                    className="flex items-center gap-2 rounded-md px-2 py-1.5 text-sm transition hover:bg-brand-50"
                  >
                    <input
                      type="checkbox"
                    ...
                    <span className="text-slate-700">
                      {v.vendorName} <span className="text-slate-400">— {v.companyName}</span>
                    </span>
                  </label>
                ))
              )}
            </div>
            {errors.vendorIds && <p className="mt-1 text-xs text-red-600">{errors.vendorIds}</p>}
          </div>
