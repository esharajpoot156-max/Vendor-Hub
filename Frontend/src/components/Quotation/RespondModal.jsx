...
const RespondModal = ({ isOpen, onClose, onSubmit, quotation }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-brand-950/50 px-4 backdrop-blur-sm">
      <div className="animate-fade-up w-full max-w-sm rounded-xl bg-white p-6 shadow-2xl">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="font-display text-lg font-semibold text-brand-950">Submit Quotation Response</h2>
        ...
        <p className="mb-4 text-sm text-slate-500">
          For <span className="font-medium text-slate-700">{quotation.title}</span> —{' '}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="mb-1 block text-sm font-medium text-slate-700">Quotation Amount (Rs.)</label>
            <input
              type="number"
              value={amount}
